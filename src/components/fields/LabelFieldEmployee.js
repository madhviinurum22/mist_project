...
export default function LabelFieldEmployee({ label, labelDir, fieldSize, option, type, placeholder,  onSelectChange, ...rest }) {
            ...
            :
                <Select  onChange={handleSelectChange} className={`mc-label-field-select ${ fieldSize || "w-md h-sm" }`} { ...rest }>
                    {option.map((item, index) => (
                        <Option key={ index } value={ item.card_uid }>{ item.username}</Option>
                    ))}
                </Select>
            }
        </Box>
